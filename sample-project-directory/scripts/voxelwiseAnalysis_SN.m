...
root_folder = ['.../sample-project-directory/data/'];%%end rootfolder with a fileseparator (\ or /)
...
topslicepath = ['.../sample-project-directory/scripts/top_slice.mat'];
load(topslicepath)
scan_key_path = ['.../sample-project-directory/scripts/scan_key.mat'];
load(scan_key_path);
...
positive_relationship = 0; %makes masks of the data selectively with + or - relationships; if set to 0, would make mask including voxels with - relationships (see mask names below)
output_file05 = [root_folder filesep '.nii'];%name for mask containing voxels with relationships where p<0.05. give this filename any name you like
output_file01 = [root_folder filesep '.nii'];%name for mask containing voxels with relationships where p<0.01
...
LowVox = -8.8257; %this excludes voxels with CNR value below -7% (if the switch exc_vox=1). These likely should be modified in a study specific manner based on the distribution of all voxels in all subjects
HiVox = 24.7862; %this excludes voxels with CNR above 24%. 

othermask = 0; %switch this on if you want to include a SN submask
VmaskFull = spm_read_vols(spm_vol([root_folder filesep 'full_SN_mask.nii']));%This is where you load the 'full' SN mask file
VmaskOther = spm_read_vols(spm_vol([root_folder filesep 'RVSTdeltaSpearman05.nii']));%replace name with new mask here <-- RVSt_dBPND_05.nii?
%VmaskOther = spm_read_vols(spm_vol([root_folder filesep 'alcohol_total_neg05.nii']));%replace name with new mask here <-- RVSt_dBPND_05.nii?
...
for s=1:length(scan_key(:,1))
    ...
    if scan_key(s,2)==1
        ...
        NMscanname = dir([root_folder 's' num2str(Subs{s}) filesep NM_filestring]); %added 's' to match file name structure
...
%added loop for submask
if othermask == 1
    for s=1:length(scan_key(:,1))
        othermaskvox=VmaskOther(VmaskFull==1);
        % Need to replace 1.0286s with 1s I think for these substance use masks
        for d=1:length(othermaskvox(:,1)) 
            if othermaskvox(d,1) > 1
                othermaskvox(d,1)=1;
            end
        end
        
        othermaskvals(:,s)=inmask_data(othermaskvox==1,s);
        avgothermask(s)=nanmean(othermaskvals(:,s)); %takes the mean of inmask_dataforall voxels where VmaskOther=1
    end
end


% for s=1:length(scan_key(:,1))
%     [x,y]=ksdensity(othermaskvals(:,s));
%     plot(y,x)
%     hold on
% end
